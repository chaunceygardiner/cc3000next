...
def configurator_loader(config_dict):
    return CC3000NextConfigurator()

def confeditor_loader():
    return CC3000NextConfEditor()
...
class CC3000NextConfigurator(weewx.drivers.AbstractConfigurator):
    def add_options(self, parser):
        super(CC3000NextConfigurator, self).add_options(parser)
    ...
    def do_options(self, options, parser, config_dict, prompt):  # @UnusedVariable
        self.driver = CC3000NextDriver(**config_dict[DRIVER_NAME])
        if options.current:
...
class CC3000NextDriver(weewx.drivers.AbstractDevice):
    ...
    def __init__(self, **stn_dict):
        ...
        port = stn_dict.get('port', CC3000Next.DEFAULT_PORT)
        ...
        self.time_change_windows = CC3000NextDriver.compose_time_change_windows(stn_dict.get('dst_periods', []))
    ...
    def _init_station_with_retries(station, max_tries):
        for cnt in range(max_tries):
            try:
                return CC3000NextDriver._init_station(station)
            except (serial.serialutil.SerialException, weewx.WeeWxIOError) as e:
    ...
    def _init_station(station):
        ...
        settings['header'] = CC3000NextDriver._parse_header(station.get_header())
    ...
    def _parse_current(values, header, sensor_map):
        return CC3000NextDriver._parse_values(values, header, sensor_map,
                                          "%Y/%m/%d %H:%M:%S")
...
class CC3000Next(object):
    ...
    def gen_records_since_ts(self, header, sensor_map, since_ts):
        ...
        for r in self.gen_records(nrec=num_records):
            pkt = CC3000NextDriver._parse_values(r[1:], header, sensor_map, "%Y/%m/%d %H:%M")
...
class CC3000NextConfEditor(weewx.drivers.AbstractConfEditor):
    @property
...
""" % (CC3000Next.DEFAULT_PORT,)

    def prompt_for_settings(self):
        print("Specify the serial port on which the station is connected, for")
        print("example /dev/ttyUSB0 or /dev/ttyS0.")
        port = self._prompt('port', CC3000Next.DEFAULT_PORT)
        return {'port': port}


# define a main entry point for basic testing.  invoke from the weewx root dir:
#
# PYTHONPATH=bin python -m user.cc3000next --help
#
# FIXME: This duplicates all of the functionality in CC3000NextConigurator.
...
if __name__ == '__main__':
    ...
    weeutil.logger.setup('cc3000next', {})
